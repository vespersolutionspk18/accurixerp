	...
	refresh: function (frm, cdt, cdn) {
		if (frm.doc.docstatus === 1) {
			...
			frm.add_custom_button(
				...
				function () {
					frappe.call({
						method: "accurix.buying.doctype.request_for_quotation.request_for_quotation.send_supplier_emails",
						freeze: true,
						args: {
						...
						(data) => {
							var w = window.open(
								frappe.urllib.get_full_url(
									"/api/method/accurix.buying.doctype.request_for_quotation.request_for_quotation.get_pdf?" +
										new URLSearchParams({
											name: frm.doc.name,
...
accurix.buying.RequestforQuotationController = class RequestforQuotationController extends (
				...
				function () {
					...
					var dialog = new frappe.ui.Dialog({
						...
						primary_action: (args) => {
							...
							accurix.utils.map_current_doc({
								method: "accurix.buying.doctype.request_for_quotation.request_for_quotation.get_item_from_material_requests_based_on_supplier",
			...
			fields: [
				{
					...
					onchange() {
						if (dialog.get_value("search_type") == "Tag") {
							frappe
								.call({
									method: "accurix.buying.doctype.request_for_quotation.request_for_quotation.get_supplier_tag",
								})
								.then((r) => {
...
extend_cscript(cur_frm.cscript, new accurix.buying.RequestforQuotationController({ frm: cur_frm }));
