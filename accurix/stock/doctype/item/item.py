...
import accurix
from accurix.controllers.item_variant import (
...
from accurix.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from accurix.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from accurix.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from accurix.stock.doctype.item_default.item_default import ItemDefault
		from accurix.stock.doctype.item_reorder.item_reorder import ItemReorder
		from accurix.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from accurix.stock.doctype.item_tax.item_tax import ItemTax
		from accurix.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from accurix.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": accurix.get_default_currency(),
			...
			for item_barcode in self.barcodes:
				...
				if item_barcode.barcode:
					...
					if item_barcode.barcode_type:
						barcode_type = convert_accurix_to_barcodenumber(
	...
	def recalculate_bin_qty(self, new_name):
		from accurix.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"accurix.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
...
def convert_accurix_to_barcodenumber(accurix_number, barcode):
	if accurix_number == "EAN":
		ean_type = {
			8: "EAN8",
			13: "EAN13",
		}
	...
	if accurix_number in convert:
		return convert[accurix_number]
...
def get_asset_naming_series():
	from accurix.assets.doctype.asset.asset import get_asset_naming_series
