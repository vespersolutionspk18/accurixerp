frappe.provide("accurix.stock_reservation");

$.extend(accurix.stock_reservation, {
	make_entries(frm, table_name) {
		accurix.stock_reservation.setup(frm, table_name);
	},

	setup(frm, table_name) {
		let parms = accurix.stock_reservation.get_parms(frm, table_name);

		accurix.stock_reservation.dialog = new frappe.ui.Dialog({
			title: __("Stock Reservation"),
			size: "extra-large",
			fields: accurix.stock_reservation.get_dialog_fields(frm, parms),
			primary_action_label: __("Reserve Stock"),
			primary_action: () => {
				accurix.stock_reservation.reserve_stock(frm, parms);
			},
		});

		accurix.stock_reservation.render_items(frm, parms);
	},
	...
	get_parms(frm, table_name) {
		...
		params["method"] = {
			"Sales Order": "delivered_qty",
			"Work Order":
				"accurix.manufacturing.doctype.work_order.work_order.make_stock_reservation_entries",
		}[frm.doc.doctype];
	...
	get_dialog_fields(frm, parms) {
		let fields = accurix.stock_reservation.fields || [];
		let qty_field = parms.qty_field;
		let dialog = accurix.stock_reservation.dialog;
	...
	unreserve_stock(frm) {
		accurix.stock_reservation.get_stock_reservation_entries(frm.doctype, frm.docname).then((r) => {
			if (!r.exc && r.message) {
				if (r.message.length > 0) {
					accurix.stock_reservation.prepare_for_cancel_sre_entries(frm, r.message);
				} else {
			...
			fields: [
				{
					...
					in_place_edit: true,
					data: [],
					fields: accurix.stock_reservation.get_fields_for_cancel(),
				},
			],
			primary_action_label: __("Unreserve Stock"),
			primary_action: () => {
				accurix.stock_reservation.cancel_stock_reservation(dialog, frm);
			},
		});
	...
	cancel_stock_reservation(dialog, frm) {
		...
		if (data.sr_entries?.length > 0) {
			frappe.call({
				method: "accurix.manufacturing.doctype.work_order.work_order.cancel_stock_reservation_entries",
				args: {
					doc: frm.doc,
