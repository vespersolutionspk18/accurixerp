...
accurix.buying = {
	setup_buying_controller: function() {
		accurix.buying.BuyingController = class BuyingController extends accurix.TransactionController {
			setup() {
				super.setup();
				...
				if(this.frm.get_field('shipping_address')) {
					this.frm.set_query("shipping_address", () => {
						if(this.frm.doc.customer) {
							...
							return accurix.queries.company_address_query(this.frm.doc)
					});
				}
			}
			...
			setup_queries(doc, cdt, cdn) {
				...
				me.frm.set_query('supplier', accurix.queries.supplier);
				me.frm.set_query('contact_person', accurix.queries.contact_query);
				me.frm.set_query('supplier_address', accurix.queries.address_query);

				me.frm.set_query('billing_address', accurix.queries.company_address_query);
				accurix.accounts.dimensions.setup_dimension_filters(me.frm, me.frm.doctype);
				...
				this.frm.set_query("manufacturer", "items", function(doc, cdt, cdn) {
					...
					return {
						query: "accurix.controllers.queries.item_manufacturer_query",
			...
			supplier() {
				var me = this;
				accurix.utils.get_party_details(this.frm, null, null, function(){
					me.apply_price_list();
				});
			}
			...
			company(){
				...
				frappe.call({
					method: "accurix.setup.doctype.company.company.get_billing_shipping_address",
					args: {
			...
			supplier_address() {
				accurix.utils.get_address_display(this.frm);
				accurix.utils.set_taxes_from_address(this.frm, "supplier_address", "supplier_address", "supplier_address");
			}
			...
			warehouse(doc, cdt, cdn) {
				...
				if(item.item_code && item.warehouse) {
					return this.frm.call({
						method: "accurix.stock.get_item_details.get_bin_details",
						child: item,
						args: {
			...
			set_from_product_bundle() {
				...
				this.frm.add_custom_button(__("Product Bundle"), function() {
					accurix.buying.get_items_from_product_bundle(me.frm);
				}, __("Get Items From"));
			}

			shipping_address(){
				var me = this;
				accurix.utils.get_address_display(this.frm, "shipping_address",
					"shipping_address_display", true);
			}

			billing_address() {
				accurix.utils.get_address_display(this.frm, "billing_address",
			...
			manufacturer(doc, cdt, cdn) {
				...
				if(row.manufacturer) {
					frappe.call({
						method: "accurix.stock.doctype.item_manufacturer.item_manufacturer.get_item_manufacturer_part_no",
						args: {
				...
				frappe.db.get_value("Item", item.item_code, fields)
					.then((r) => {
						if (r.message && (r.message.has_batch_no || r.message.has_serial_no)) {
							...
							new accurix.SerialBatchPackageSelector(
								me.frm, item, (r) => {
									if (r) {
...
accurix.buying.link_to_mrs = function(frm) {
	frappe.call({
...
accurix.buying.get_default_bom = function(frm) {
	$.each(frm.doc["items"] || [], function(i, d) {
		if (d.item_code && d.bom === "") {
			return frappe.call({
				type: "GET",
				method: "accurix.stock.get_item_details.get_default_bom",
				args: {
...
accurix.buying.get_items_from_product_bundle = function(frm) {
	var dialog = new frappe.ui.Dialog({
		...
		primary_action(args){
			...
			return frappe.call({
				type: "GET",
				method: "accurix.stock.doctype.packed_item.packed_item.get_items_from_product_bundle",
				args: {
					row: {
