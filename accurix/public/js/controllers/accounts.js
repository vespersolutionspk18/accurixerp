...
// get tax rate
frappe.provide("accurix.taxes");

accurix.accounts.taxes = {
			...
			onload: function(frm) {
				if(frm.get_field("taxes")) {
					frm.set_query("account_head", "taxes", function(doc) {
						...
						return {
							query: "accurix.controllers.queries.tax_account_query",
							filters: {
		...
		frappe.ui.form.on(doctype, {
			account_head: function(frm, cdt, cdn) {
				...
				if(!d.charge_type && d.account_head){
					...
					frappe.call({
						type:"GET",
						method: "accurix.controllers.accounts_controller.get_tax_rate",
...
accurix.accounts.payment_triggers = {
...
accurix.accounts.pos = {
	...
	get_payment_mode_account: function(frm, mode_of_payment, callback) {
		...
		return  frappe.call({
			method: "accurix.accounts.doctype.sales_invoice.sales_invoice.get_bank_cash_account",
			args: {
