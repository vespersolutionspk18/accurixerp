...
frappe.provide("accurix.queries");
$.extend(accurix.queries, {
	user: function () {
	...
	item: function (filters) {
		var args = { query: "accurix.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "accurix.controllers.queries.bom" };
	},

	task: function () {
		return { query: "accurix.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "accurix.controllers.queries.employee_query" };
	},
	...
	get_filtered_dimensions: function (doc, child_fields, dimension, company) {
		...
		return {
			query: "accurix.controllers.queries.get_filtered_dimensions",
			filters: {
				dimension: dimension,
				account: account,
				company: company,
...
accurix.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
accurix.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = accurix.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "accurix.controllers.queries.warehouse_query" });
