...
accurix.landed_cost_taxes_and_charges.setup_triggers("Subcontracting Order");
...
frappe.ui.form.on("Subcontracting Order", {
	...
	set_queries: (frm) => {
		...
		frm.set_query("supplier_address", accurix.queries.address_query);

		frm.set_query("billing_address", accurix.queries.company_address_query);

		frm.set_query("shipping_address", () => {
			return accurix.queries.company_address_query(frm.doc);
		});
	},
	...
	update_subcontracting_order_status(frm, status) {
		frappe.call({
			method: "accurix.subcontracting.doctype.subcontracting_order.subcontracting_order.update_subcontracting_order_status",
			args: {
				sco: frm.doc.name,
				status: status,
			},
...
accurix.buying.SubcontractingOrderController = class SubcontractingOrderController {
	setup() {
	...
	make_subcontracting_receipt() {
		frappe.model.open_mapped_doc({
			method: "accurix.subcontracting.doctype.subcontracting_order.subcontracting_order.make_subcontracting_receipt",
			frm: cur_frm,
...
extend_cscript(cur_frm.cscript, new accurix.buying.SubcontractingOrderController({ frm: cur_frm }));
